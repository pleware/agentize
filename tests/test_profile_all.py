...
def test_mcp_keys_say_which_identity_owns_them(project: Path):
    ...
    assert mcp_keys(project) == ["agentize-agent.pg", "agentize-human.pg", "agentize-php.pg"]
    assert mcp_keys(project, "opencode.json") == [
        "agentize-agent.pg",
        "agentize-human.pg",
        "agentize-php.pg",
    ]
...
def test_one_identity_prunes_the_qualified_files(project: Path):
    ...
    assert mcp_keys(project) == ["agentize-pg"]
