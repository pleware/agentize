#!/bin/sh
# Plant agentize launchers in the current project (macOS).
...
if [ "${AGENTIZE_SKIP_OS_CHECK:-}" != 1 ] && [ "$(uname -s)" != Darwin ]; then
  printf '%s\n' "agentize: this installer is for macOS; on Linux use install.sh" >&2
  exit 1
fi
...
uv_asset() {
  ...
  case "$arch" in
    ...
    *) die "no uv build for macOS/$arch" ;;
  esac
}
...
ensure_uv() {
  ...
  if [ "${AGENTIZE_UV_SKIP_PATH_WRITE:-}" != 1 ]; then
    ...
    append_path_line "$HOME/.zprofile" "export PATH=\"$UV_BIN:\$PATH\""
  fi
